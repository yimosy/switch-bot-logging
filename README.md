...
[Open-Meteo](https://open-meteo.com/)(無料・APIキー不要)から外気データを取得して、専用の `weather_measurements` テーブルに記録できます。気温・湿度のほか、体感温度・降水量・天気コード・雲量・気圧・風速/風向なども保存されます。ダッシュボードには「外気」として破線で表示され、室内との比較ができます。

旧バージョンで `measurements` テーブルに擬似デバイスとして保存していた外気データは、ロガー起動時に自動で `weather_measurements` へ移行されます。
...
## テーブル構造(weather_measurements)

Open-Meteoの外気データ専用テーブルです。
...
| `weather_code` | INTEGER | 天気コード([WMOコード](https://open-meteo.com/en/docs#weather_variable_documentation)) |
| `cloud_cover` | INTEGER | 雲量(%) |
| `pressure_msl` | FLOAT | 海面気圧(hPa) |
| `surface_pressure` | FLOAT | 地上気圧(hPa) |
| `wind_speed` | FLOAT | 風速(km/h) |
| `wind_direction` | INTEGER | 風向(度) |
| `wind_gusts` | FLOAT | 最大瞬間風速(km/h) |
| `is_day` | INTEGER | 昼=1 / 夜=0 |
| `raw_status` | TEXT | APIレスポンス全体(JSON) |
